...
open class ViewModelFactory(
    ...
    private val delegateFactory: ViewModelProvider.Factory? = null
) : ViewModelProvider.NewInstanceFactory() {
    @Suppress("UNCHECKED_CAST")
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        return if (application != null && AbsAndroidViewModel::class.java.isAssignableFrom(
                modelClass
            )
        ) {
            try {
...
fun initAbsViewModel(owner: ComponentActivity?, viewModel: ViewModel) {
    if (owner is AbsActivity && viewModel is AbsViewModel) {
        ...
        viewModel._tipDialog.observe(owner) {
            it?.build()?.show(owner.supportFragmentManager)
        }
